...
class Person {
 public:
  ...
  bool operator==(const Person& oth) const {
    return m_sex == oth.m_sex && m_name == oth.m_name && m_age == oth.m_age;
  }
};
...
sylar::ConfigVar<std::map<std::string, std::vector<Person>>>::ptr
    g_person_map_vector = sylar::Config::Lookup(
...
void test_class() {
  SYLAR_LOG_INFO(SYLAR_LOG_ROOT) << "before : " << g_person->getVal().toString()
                                 << " - " << g_person->toString();
#define XX_PM(g_val, prefix)                                             \
  ...
  g_person->addListener(10,[](const Person& old_value, const Person& new_value){
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT)<< "old_value = "<<old_value.toString()<<"new_value"<<new_value.toString()<<std::endl;
  });
  ...
  YAML::Node root =
      ...
      YAML::LoadFile("/home/geray/cplusplus/workspace/bin/log.yaml");
  sylar::Config::LoadFromYaml(root);
  SYLAR_LOG_INFO(SYLAR_LOG_ROOT) << "after : " << g_person->getVal().toString()
                                 << " - " << g_person->toString();
  SYLAR_LOG_INFO(SYLAR_LOG_ROOT)
      << "after : " << g_person_map_vector->toString();
